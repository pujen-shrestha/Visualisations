...
# This visualisation was based on some cool plots I saw on Twitter by Albert Rapp find on Github: https://github.com/AlbertRapp/PublicTidyTuesday/blob/main/2022_w28_euroflights/2022_w28_euroflights.qmd
...
camcorder::gg_record(
  dir = here::here('2022_w28_covid/img'),
  width = 30,
  height = 20,
  units = 'cm',
  dpi = 600
...
covid_cases <- covid_cases %>% 
  mutate(year = year(date),
         month = month(date),
         quarter = case_when(month == 1 | month == 2 |
                               month == 3 ~ 1,
                             month == 4 | month == 5 |
                               month == 6 ~ 2,
                             month == 7 | month == 8 |
                             ...
                             month == 10 | month == 11 |
                               month == 12 ~ 4),
         year_quarter = paste0(year, " Quarter ", quarter),
         new_cases = if_else(is.na(new_cases), 0, new_cases))
```


```{r}
# Get flights total in state by year
...
#   filter(state != "Asia") %>%
#   filter(state != "Europe") %>%
#   filter(state != "North America") %>%
#   filter(state != "South America") %>%
#   filter(state != "Africa") %>%
#   filter(state != "High income") %>%
#   filter(state != "Upper middle income") %>%
#   filter(state != "Lower middle income") %>%
#   filter(state != "European Union") %>%
#   filter(state != "Lower middle income")
...
bump_chart_basic <- country_rank_by_year_quarter %>% 
  ...
  geom_text(
    data = country_rank_by_year_quarter %>% filter(year == "2022", state %in% todays_top),
    aes(label = rank),
    hjust = 0,
    nudge_x = 0.1
  ) +
...
combined_plot <- percentage_bars_plot +
  ...
  annotate(
    'text',
    x = c(2020 - bar_width, 2022 + bar_width),
    y = c(1.1, 1.1),
    label = c(2020, 2022),
    hjust = c(0, 1),
    vjust = 1,
